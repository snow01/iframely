    ...
    function findPluginMethods(pluginId, loadedParams, pluginsUrlMatches, usedMethods, usedParams, methods, scannedPluginsIds, usedDomains, mandatoryParams) {
        ...
        * */
        ...
        if (!mandatoryParams) {
            ...
            if (mixins) {
                if (CONFIG.AUTO_INCLUDE_MIXINS) {
                    for(var i = 0; i < CONFIG.AUTO_INCLUDE_MIXINS.length; i++) {
                        findPluginMethods(CONFIG.AUTO_INCLUDE_MIXINS[i], loadedParams, pluginsUrlMatches, usedMethods, usedParams, methods, scannedPluginsIds, usedDomains);
                    }
                }
